...
const mockLogseq = Object.assign(new EventEmitter(), {
  ...
  provideUI: (config: any) => {
    ...
    if (!targetElement) {
      logger.warn(`Target element '${targetPath}' not found, using body instead`);
      targetElement = doc.body;
    }

    if (!config.key) {
      logger.warn('No key provided for provideUI, generating random key');
  ...
  DB: {
    ...
    datascriptQuery: async (query: string, ...inputs: any[]) => {
      ...
      try {
        ...
        // V2 查询格式：返回 [[块对象, status-title], [块对象, status-title], ...]
        // 查询格式是 [:find (pull ?b ...) ?status-title :where ...]
        const results = blocksWithParent.map(child => {
          // 从 properties 中获取状态，如果没有则默认为 'todo'
          const status = child.properties?.status || child.status || 'todo';
          return [child, status];
        });
        return results;
      } catch (error) {
        logger.error('[Mock DB] datascriptQuery error:', error);
        return [];
      }
    },
    onChanged: () => () => {}
