...
const Tooltip: React.FC<TooltipProps> = ({ data, position }) => {
 ...
 return (<div className="heatmap-tooltip" style={{
 position: 'fixed',
 left: position.x + 12,
 top: position.y - 60,
...
const HeatmapCell: React.FC<HeatmapCellProps> = ({ date, value, maxValue, color, isEmpty, size = 'small', onClick, }) => {
 ...
 const sizeMap = {
 small: { width: 12, height: 12, gap: 2 },
 medium: { width: 20, height: 20, gap: 3 },
 large: { width: 32, height: 40, gap: 4 },
 };
 const { width, height } = sizeMap[size];
 return (<>
 <div className={`heatmap-cell ${isEmpty ? 'empty' : ''} ${isHovered ? 'hovered' : ''}`} style={{
 width: `${width}px`,
 height: `${height}px`,
 ...
 }} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onClick={handleClick} title={isEmpty ? undefined : `${date}: ${value} blocks`}/>
 {isHovered && tooltipPosition && (<Tooltip data={{
 date,
 count: value,
 percentage,
 maxValue,
