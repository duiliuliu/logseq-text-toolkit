...
const testCases: TestCase[] = [
  ...
  // 注意：多层混合格式会被整体包裹，原有格式保留
  { name: '多层混合格式', item: toolbarItems.find(i => i.id === 'wrap-purple-text')!, input: '==**高亮加粗**==', expected: '[:span.purple "==**高亮加粗**=="]', category: 'normal', note: '外层hiccup包裹整体，Markdown标记保留' },
  ...
  // 空文本 - 这些情况应该直接返回空
  { name: '边界空文本', item: toolbarItems.find(i => i.id === 'wrap-bold')!, input: '', expected: '', category: 'edge', note: '空文本直接返回空' },
  { name: '边界仅换行', item: toolbarItems.find(i => i.id === 'wrap-red-text')!, input: '\n', expected: '', category: 'edge', note: '仅换行符过滤后为空' },
  { name: '边界仅空格', item: toolbarItems.find(i => i.id === 'wrap-blue-text')!, input: '   ', expected: '', category: 'edge', note: '仅空格会被空格处理' },
  { name: '边界仅换行和空格', item: toolbarItems.find(i => i.id === 'wrap-green-text')!, input: '\n  \n', expected: '', category: 'edge', note: '换行和空格过滤后为空' },
  
  // 纯格式标记 - 这些会保留原始格式标记
  { name: '边界纯粗体标记', item: toolbarItems.find(i => i.id === 'wrap-red-text')!, input: '****', expected: '[:span.red ****]', category: 'edge', note: '空粗体标记保留' },
