...
**为什么需要引号？**
当文本本身包含引号时，需要用引号包裹并对内部引号进行转义，原因如下：
1. **明确边界** - 需要明确告诉hiccup解析器哪些是内容，哪些是语法分隔符
2. **防止解析错误** - 如果文本中有`"`而不用引号包裹，解析器会以为这是内容的结束
3. **保持语义完整** - 转义的引号会被正确还原为原文本中的引号字符

例如：
- `[:span Hello"World]` 会导致解析错误，因为`"`被当作字符串结束符
- `[:span "Hello\"World"]` 会被正确解析为内容`Hello"World`
