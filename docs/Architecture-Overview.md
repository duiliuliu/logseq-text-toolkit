...
│ │ StatsCalculator                                            │  │
│ │ ┌──────────────────────┐ ┌──────────────────────┐        │  │
│ │ │ calculateProgress()  │ │ getStatusCounts()    │        │  │
│ │ │ (计算进度百分比)       │ │ (获取各状态数量)      │        │  │
│ │ └──────────────────────┘ └──────────────────────┘        │  │
│ └────────────────────────────────────────────────────────────┘  │
│                                                                  │
...
│ ┌─────────────────────────────────────────────────────────────┐ │
│ │ MiniCircleProgress - 迷你圆形进度条                         │ │
│ │ DotMatrixProgress - 点阵进度显示                           │ │
│ │ StatusCursorProgress - 状态游标进度                        │ │
│ │ ProgressCapsule - 胶囊型进度条                             │ │
...
**进度计算流程**:
1. **数据获取** → 通过 Datascript 查询获取任务块
2. **状态统计** → 统计 TODO/DONE/CANCELLED 数量
3. **进度计算** → 根据状态计算完成百分比
4. **UI 渲染** → 根据配置渲染对应的进度组件
...
│ │ ColorCalculator                                             │  │
│ │ ┌──────────────────────┐ ┌──────────────────────┐        │  │
│ │ │ generateGradient()   │ │ getColorByValue()    │        │  │
│ │ │ (生成渐变色)          │ │ (根据值获取颜色)      │        │  │
│ │ └──────────────────────┘ └──────────────────────┘        │  │
│ └────────────────────────────────────────────────────────────┘  │
│                                                                  │
...
| 函数 | 用途 | 实现方式 |
|------|------|----------|
| `generateIndigoGradient()` | 生成靛蓝色渐变 | 线性插值计算 RGB 值 |
| `getColorByValue()` | 根据值获取颜色 | 百分比映射到渐变色阶 |
| `getPercentage()` | 计算百分比 | (value / maxValue) * 100 |
...
│ ┌─────────────────────────────────────────────────────────────┐ │
│ │ Templates:                                                   │ │
│ │ - GTDWorkReview    - GTD工作回顾                            │ │
│ │ - MinimalDashboard - 极简仪表盘                             │ │
│ │ - OKRReview        - OKR回顾                                │ │
│ │ - BulletJournal    - 子弹笔记                               │ │
│ │ - StudySummary     - 学习总结                               │ │
│ └─────────────────────────────────────────────────────────────┘ │
...
| 模板 | 用途 | 适用场景 |
|------|------|----------|
| GTDWorkReview | GTD 工作回顾 | 每日/每周工作总结 |
| MinimalDashboard | 极简仪表盘 | 快速概览数据 |
| OKRReview | OKR 回顾 | 目标跟踪和评估 |
| BulletJournal | 子弹笔记 | 日记本格式 |
| StudySummary | 学习总结 | 学习进度追踪 |
